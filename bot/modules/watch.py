...
def _watch(bot, message, isZip=False, isLeech=False, multi=0):
    ...
    if 'entries' in result:
        ...
        buttons.sbutton("Audiolar", f"qu {msg_id} audio t")
        buttons.sbutton("Eng tiniq video", f"qu {msg_id} {best_video} t")
        buttons.sbutton("Eng tiniq audio", f"qu {msg_id} {best_audio} t")
        ...
        bmsg = sendMarkup('Video sifatini tanlang:', bot, message, YTBUTTONS)
    else:
        ...
        buttons.sbutton("Audiolar", f"qu {msg_id} audio")
        buttons.sbutton("Eng tiniq video", f"qu {msg_id} {best_video}")
        buttons.sbutton("Eng tiniq audio", f"qu {msg_id} {best_audio}")
        buttons.sbutton("Bekor qilish", f"qu {msg_id} cancel")
...
def _qual_subbuttons(task_id, qual, msg):
    ...
    editMessage(f"Video bitreytini tanlang <b>{qual}</b> uchun:", msg, SUBBUTTONS)
...
def _audio_subbuttons(task_id, msg, playlist=False):
    ...
    buttons.sbutton("Bekor qilish", f"qu {task_id} cancel")
    SUBBUTTONS = InlineKeyboardMarkup(buttons.build_menu(2))
    editMessage(f"Audio {i} bitreytini tanlang:", msg, SUBBUTTONS)
...
def select_format(update, context):
    ...
    try:
        ...
        return editMessage("Bu eski vazifa", msg)
    uid = task_info[1]
    if user_id != uid and not CustomFilters._owner_query(user_id):
        return query.answer(text="Bu vazifa siz uchun emas!", show_alert=True)
    elif data[2] == "dict":
        query.answer()
        qual = data[3]
        ...
        query.answer()
        return editMessage('Video sifatini tanlang:', msg, task_info[4])
    elif data[2] == "audio":
        query.answer()
        if len(data) == 4:
            playlist = True
        ...
        query.answer()
        editMessage('Vazifa bekor qilindi.', msg)
    else:
        query.answer()
...
def _auto_cancel(msg, msg_id):
    ...
    try:
        ...
        editMessage('Vaqt tugadi! Vazifa bekor qilindi.', msg)
    except:
        pass
